...
export function getDefaultLanguage(options) {
    const { order, lookupQuerystring, lookupCookie, lookupLocalStorage, lookupSessionStorage, } = options;
    let language = "";
...
export function saveSelectedLanguage(lang, detectOptions) {
    const { caches, lookupQuerystring, lookupCookie, lookupLocalStorage, lookupSessionStorage, } = detectOptions;
    caches.forEach((key) => {
        if (key === "localStorage") {
